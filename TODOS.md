...
- [ ] **install-base / control-plane URL mismatch.** `pairing-token/route.ts` builds the install one-liner from `NEXT_PUBLIC_APP_URL` (the dashboard origin that serves `/install.sh`) but does NOT pass `IMSG_CONTROL_PLANE_URL`, so the device falls back to its own default (`http://localhost:8080`). On any real deploy where the dashboard and control plane are different hosts, the device pairs against the wrong control plane → `invalid_pairing_token`. Fix: thread the control-plane base into the install one-liner (`IMSG_CONTROL_PLANE_URL=${base}`).
