...
- [ ] **install-base / control-plane URL mismatch.** `pairing-token/route.ts` falls back to `NEXT_PUBLIC_APP_URL` (doesn't serve `/install.sh`) and `install.sh` hardcodes the control plane to `https://message.prbe.ai`. A non-default deploy pairs against the wrong control plane → `invalid_pairing_token`. Fix: require an explicit control-plane base; pass `IMSG_CONTROL_PLANE_URL=${base}` into the install one-liner.
