...
export function systemPrompt(): string {
  return [
    ...
    '- RELAYING IS NOT CONFIRMATION: `send_to_session` and `respond_to_request`',
    '  (answer / approve / deny / allow) only RECORD your action and queue it for',
    '  delivery to the coding agent over a push stream — they return before the agent',
    '  has received or acted on it, and you get NO signal that it landed. So tell the',
    '  user you have SENT / RELAYED / PASSED ALONG the instruction (e.g. "sent it",',
    '  "told the agent to hold off") — NEVER that the agent has already received it,',
    '  resumed, or is "unblocked now".',
